...
def get_args(description='Disentangled Representation Learning for Text-Video Retrieval'):
    ...
    parser.add_argument("--query_number", default=16, type=int)
    parser.add_argument("--cross_att_layer", default=3, type=int)
    parser.add_argument("--add_query_score_for_eval", default=0, type=int)
    parser.add_argument("--query_share", default=1, type=int)
    parser.add_argument("--cross_att_share", default=1, type=int)
    parser.add_argument("--loss2_weight", default=0.5, type=float)
...
def main():
    ...
    if args.do_train:
        ...
        eval_epoch(args, model, test_dataloader, args.device)
        # ckpts = os.listdir("ckpt/msrvtt/ablation/8query_intra_consistency_MSE_0.0001_inter_diversity_0.1margin_both_3cross_add_query_sim_query_shared_cross_att_shared_without_weight")
        # for c in ckpts:
        #     if c == 'log.txt':
        #         continue
        #     args.init_model = "ckpt/msrvtt/ablation/8query_intra_consistency_MSE_0.0001_inter_diversity_0.1margin_both_3cross_add_query_sim_query_shared_cross_att_shared_without_weight/" + c
        #     model = build_model(args)
        #     eval_epoch(args, model, test_dataloader, args.device)
...
def _run_on_single_gpu(args, model, t_mask_list, v_mask_list, t_feat_list, v_feat_list, cls_list,
    ...
    with torch.no_grad():        
        for idx1, (t_mask, t_feat, cls) in enumerate(zip(batch_t_mask, batch_t_feat, batch_cls_feat)):
            ...
            for idx2, (v_mask, v_feat) in enumerate(zip(batch_v_mask, batch_v_feat)):
                ...
                logits_2, _, _ = model._score(t_feat, cls, v_feat, t_mask, v_mask)
                logits_1_2 = logits + args.loss2_weight * logits_2
                
                each_row_1.append(logits.cpu().detach().numpy())
                each_row_2.append(logits_2.cpu().detach().numpy())
                each_row_1_2.append(logits_1_2.cpu().detach().numpy())

            each_row_1 = np.concatenate(tuple(each_row_1), axis=-1)
            each_row_2 = np.concatenate(tuple(each_row_2), axis=-1)
            each_row_1_2 = np.concatenate(tuple(each_row_1_2), axis=-1)
            
            sim_matrix_1.append(each_row_1)
            sim_matrix_2.append(each_row_2)
            sim_matrix_1_2.append(each_row_1_2)
            
    sim_matrix_1 = np.concatenate(tuple(sim_matrix_1), axis=0)
    sim_matrix_2 = np.concatenate(tuple(sim_matrix_2), axis=0)
    sim_matrix_1_2 = np.concatenate(tuple(sim_matrix_1_2), axis=0)
    ...
    t2v_normalized, v2t_normalized = None, None
    return sim_matrix_1, sim_matrix_2, sim_matrix_1_2, t2v_normalized, v2t_normalized
...
def eval_epoch(args, model, test_dataloader, device):
    ...
    with torch.no_grad():
        sim_matrix_1, sim_matrix_2, sim_matrix_1_2, t2v_sim_matrix, v2t_sim_matrix = _run_on_single_gpu(args, model, batch_mask_t, batch_mask_v, batch_feat_t, batch_feat_v, batch_cls, batch_mask_train_t, batch_mask_train_v, batch_feat_train_t, batch_feat_train_v, batch_train_cls)
    ...
    if multi_sentence_:  
        ...
        logger.info("sim matrix size: {}, {}".format(sim_matrix_1.shape[0], sim_matrix_1.shape[1]))
        tv_metrics = compute_metrics(sim_matrix_1_2, re_ranking=False, sim=sim_matrix_2)
        vt_metrics = compute_metrics(sim_matrix_1_2.T, re_ranking=False, sim=sim_matrix_2.T)

        tv_metrics2 = compute_metrics(sim_matrix_2, re_ranking=False, sim=sim_matrix_2)
        vt_metrics2 = compute_metrics(sim_matrix_2.T, re_ranking=False, sim=sim_matrix_2.T)

        tv_metrics12 = compute_metrics(sim_matrix_1_2, re_ranking=False, sim=sim_matrix_2)
        vt_metrics12 = compute_metrics(sim_matrix_1_2.T, re_ranking=False, sim=sim_matrix_2.T)
        logger.info('\t Length-T: {}, Length-V:{}'.format(len(sim_matrix_1), len(sim_matrix_1[0])))
    ...
    logger.warning("Score 1 Results:")
    ...
    logger.warning("Score 2 Results:")

    logger.info("Text-to-Video: R@1: {:.1f} - R@5: {:.1f} - R@10: {:.1f} - R@50: {:.1f} - Median R: {:.1f} - Mean R: {:.1f}".
                format(vt_metrics2['R1'], vt_metrics2['R5'], vt_metrics2['R10'], vt_metrics2['R50'], vt_metrics2['MR'], vt_metrics2['MeanR']))
    logger.info("Video-to-Text: R@1: {:.1f} - R@5: {:.1f} - R@10: {:.1f} - R@50: {:.1f} - Median R: {:.1f} - Mean R: {:.1f}".
                format(vt_metrics2['R1'], vt_metrics2['R5'], vt_metrics2['R10'], vt_metrics2['R50'], vt_metrics2['MR'], vt_metrics2['MeanR']))


    logger.warning("Score 1+2 Results:")

    logger.info("Text-to-Video: R@1: {:.1f} - R@5: {:.1f} - R@10: {:.1f} - R@50: {:.1f} - Median R: {:.1f} - Mean R: {:.1f}".
                format(tv_metrics12['R1'], tv_metrics12['R5'], tv_metrics12['R10'], tv_metrics12['R50'], tv_metrics12['MR'], tv_metrics12['MeanR']))
    logger.info("Video-to-Text: R@1: {:.1f} - R@5: {:.1f} - R@10: {:.1f} - R@50: {:.1f} - Median R: {:.1f} - Mean R: {:.1f}".
                format(vt_metrics12['R1'], vt_metrics12['R5'], vt_metrics12['R10'], vt_metrics12['R50'], vt_metrics12['MR'], vt_metrics12['MeanR']))
